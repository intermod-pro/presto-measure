...
class T2_cavity_coherent(Base):
    ...
    def run(
        ...
        with pulsed.Pulsed(
            ...
            **CONVERTER_CONFIGURATION,
            ...
            pls.setup_scale_lut(
                self.cavity_port, group=0, scales=[self.cavity_amp, -self.cavity_amp]
            )
            ...
            for delay in self.delay_arr:
                pls.select_scale(T, 0, self.cavity_port, group=0)
                pls.output_pulse(T, cavity_pulse)  # displace cavity
                T += self.cavity_duration
                T += delay  # increasing delay
                pls.select_scale(T, 1, self.cavity_port, group=0)
                pls.output_pulse(T, cavity_pulse)  # displace cavity back
                T += self.cavity_duration
    ...
    @classmethod
    def load(cls, load_filename: str) -> "T2_cavity_coherent":
